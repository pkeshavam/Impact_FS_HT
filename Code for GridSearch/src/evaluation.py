...
def plot_confusion_matrix(y_true, y_pred, current_CSV, model_name, tuned, selected_model, save_dir="Plots"):
    ...
    # Create a DataFrame with the confusion matrix and the class labels
    conf_matrix_df = pd.DataFrame(conf_matrix)
    
    # Save confusion matrix to Excel
    excel_filename = f"{model_name.lower().replace(' ', '_')}_{'with_tuning' if tuned else 'no_tuning'}_confusion_matrix.xlsx"
    conf_matrix_df.to_excel(os.path.join(save_dir, excel_filename))
    print(f"Confusion matrix saved to {os.path.join(save_dir, excel_filename)}.")
...
def evaluate(directory, X_test, y_test, current_CSV, tuning, selected_model, save_metrics_to, save_dir="Plots"):
    ...
    # Save entire classification report
    report = classification_report(y_test, y_pred, output_dict=True)
    metrics_file = os.path.join("Metrics", f"{selected_model}_{current_CSV}_metrics.csv")
    metricsInfo = {**report}
    pd.DataFrame(metricsInfo).T.to_csv(metrics_file)
